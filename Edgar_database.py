...
class EdgarDatabase:
    ...
    #-------------------THEN IN THE HANDLER THE DATA CAN BE SORTED AND COMBINED AS NEEDED MUSH EASIER THAN IF ITS CLUMPED TOGETHER----------- 
    def getFilingViewerResults(self,  values):
        sql = '''SELECT 
                    name_of_issuer, title_of_class, cusip, value, shares_principle_amount, 
                    shares_or_principle, put_long_call, investment_discretion, other_manager
                FROM 
                    InfoTable13FData
                WHERE
                    '''
        sql=sql + ""
    def getNumberOfResults(self, values):
        #what is needed
            #share id of shares that are from the filing with the matching ascession number
        sql = "SELECT shareissue_id FROM infoTable13FData WHERE "
    ...
    def getPositionDetails(self, cusip,  acc_num,  put_call_long, other_manager):
        sql = ''' SELECT    
                    ...
                    AND (  InfoTable13FData.other_manager_sequence_numbers = %(other_manager)s)
                    '''
        self.cursor.execute(sql,  {'acc_num_1': acc_num, 'cusip_var': str(cusip),  'PCL': put_call_long, 'other_manager': str(other_manager)})
        return self.cursor.fetchall()
    ...
    def insertFilingEntity(self, data):
        ...
        self.cursor.execute(sql,  {  'entity_name':data[0],  'cik' : str(data[1]),  'irs_number'  : data[2],  'state_of_incorperation':data[3]} )
        self.commit()
    ...
    def insertFiling(self, entity_id, data,  is_reported_by_another_manager):
        ...
        if "not" in data[7]:
            ...
            is_amend = True
        self.cursor.execute(sql,  { 'filing_entity_id':entity_id,  'accession_number':str(data[0]),  'form_type':data[1],  'sec_file_number':data[6],  'period_of_report':data[2],  'filed_as_of_date':data[3],  'date_as_of_change':data[4],  'effectiveness_date':data[5],  'is_amendment':is_amend,  'amendment_type':data[7],  'is_reported_by_another_manager':is_reported_by_another_manager})
        self.commit()
    ...
    def insertInfoTable13F(self, filing_id,  data):
        ...
        self.cursor.execute(sql,  { 'filing_id': filing_id,  'other_managers': data[0],  'table_entry_total': data[1],  'table_value_total': data[2],  'is_confidential_omitted': data[3]})
...
#        print("cusip:" + str(data[2]))
        ...
        self.cursor.execute(sql, { 'infotable_id':infotable_id,  'cusip':str(data[2]),  'value':data[3],  'shares_principle_amount':data[4],  'shares_or_principle':data[5],  'put_long_call':data[6],  'investment_discretion':data[7],  'other_manager_sequence_numbers':data[8],  'sole_voting_authority':data[9],  'shared_voting_authority':data[10],  'none_voting_authority':data[11]} )
    ...
    def insertEntityBusinessAddress(self, entity_id,  data):
        ...
        self.cursor.execute(sql, {  'filing_entity_id':entity_id,  'street_1':data[0],  'street_2':data[1],  'city':data[2],  'state':data[3],  'zip':data[4],  'business_phone_number':data[5]})
    ...
    def insertEntityMailingAddress(self, entity_id,  data):
        ...
        self.cursor.execute(sql,  { 'filing_entity_id':entity_id,  'street_1':data[0],  'street_2':data[1],  'city':data[2],  'state':data[3],  'zip':data[4]})
    ...
    def insertFilingSignatureBlock(self, filing_id, data):
        ...
        self.cursor.execute(sql,  { 'filing_id':filing_id,  'name_of_signer':data[0],  'title_of_signer':data[1],  'phone_number':data[2],  'signature':data[3],  'city':data[4],  'state_or_county':data[5],  'signature_date':data[6]})
    ...
    def insertTrackedEntityFiling(self, filing_entity_id, filing_type,  last_file_date):
        ...
        self.cursor.execute(sql,  { 'filing_entity_id':filing_entity_id,  'filing_type':filing_type,  'last_file_date':last_file_date})
